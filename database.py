...
class Database:
    ...
    async def connect(self):
        ...
        await self.execute(
            "CREATE TABLE IF NOT EXISTS users (`ID` INTEGER PRIMARY KEY AUTOINCREMENT , `chat_id` BIGINT(15) NOT NULL, `lastmsg` INT(8) DEFAULT 0, `defaultTime` INTEGER DEFAULT 2, `defaultWelcome` VARCHAR DEFAULT '0');",
            [], commit=True)
        await self.execute(
            "CREATE TABLE IF NOT EXISTS channels (`ID` INTEGER PRIMARY KEY AUTOINCREMENT, `chat_id` BIGINT(15) NOT NULL, `tempoAttesa` INTEGER, `welcomePost` VARCHAR, `goodbyePost` VARCHAR DEFAULT '0', `userid` BIGINT);",
            [], commit=True)
    ...
    async def addchannel(self, chat_id: int, userid: int, welcome, time):
        fc = await self.execute('SELECT * FROM channels WHERE chat_id = ?', (chat_id,), fetch=1)
        if not fc:
            await self.execute('INSERT INTO channels (chat_id, userid, tempoAttesa, welcomePost) VALUES (?, ?, ?, ?)',
                               (chat_id, userid, time, welcome), commit=True)
        return True if not fc else False
    ...
    async def getChannelCheckAdmin(self, chat_id, userid):
        return await self.execute('SELECT chat_id FROM channels WHERE userid= ? AND chat_id = ?', (userid, chat_id,),
                                  fetch=1)
    ...
    async def getDefaultWelcome(self, userid):
        return await self.execute('SELECT defaultWelcome FROM users WHERE chat_id = ?', (userid,), fetch=1)
    ...
    async def updateWelcome(self, nuovoWelcome: int, chat_id: int):
        await self.execute('UPDATE channels SET welcomePost = ? WHERE chat_id = ?', (nuovoWelcome, chat_id,),
                           commit=True)

    async def updateDefaultWelcome(self, nuovoWelcome: int, chat_id: int):
        await self.execute('UPDATE users SET defaultWelcome = ? WHERE chat_id = ?', (nuovoWelcome, chat_id,),
                           commit=True)
    ...
    async def updateGoodbye(self, nuovoGoodbye: str, chat_id: int):
        await self.execute('UPDATE channels SET goodbyePost = ? WHERE chat_id = ?', (nuovoGoodbye, chat_id,), commit=True)

    async def getGoodbye(self, chat_id: int):
        return await self.execute('SELECT goodbyePost FROM channels WHERE chat_id = ?', (chat_id,), fetch=1)
